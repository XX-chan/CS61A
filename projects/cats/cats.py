...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_p=[x for x in paragraphs if select(x)]
    if k>=len(select_p):
        return ''
    return select_p[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    def select(paragraph):
        ...
        return any([x in word for x in subject ])
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if typed_words==[]:
        ...
        accuracy_word=[t for t ,s in zip(typed_words , source_words) if t==s]
        return round ((len(accuracy_word)/len(typed_words))*100,2)
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        closed_word=min (word_list,key=lambda word:diff_function(typed_word,word,limit))
        if diff_function(typed_word,closed_word,limit)>limit:
            return typed_word
        return closed_word
...
def report_progress(typed, source, user_id, upload):
    ...
    for t,s in zip(typed,source):
        if t==s:
            ...
            break
    progress=correct_sofar/len(source)
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    faster=[[] for _ in player_indices]
    for j in word_indices:
       faster_player=min(player_indices,key=lambda player:get_time(times,player,j))
       faster[faster_player].append(words[j])
    return faster
    # END PROBLEM 10
